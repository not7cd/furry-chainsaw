...
$(function() {
  ...
  $("[name=sliderDate]").change(function() {
    ...
    fetchData(
      "https://launchlibrary.net/1.4.1/launch/" +
        newval +
        "-01-01" +
        "/" +
        newval +
  ...
  fetchData(
    "https://launchlibrary.net/1.4.1/launch/" +
      ...
      "/" +
      new Date().getFullYear() +
      "-12-31?limit=200&status=1,3,4,5,6,7",
    processLaunches
  );
...
function processLaunches(data) {
  ...
  globe.goTo(
    new WorldWind.Location(
      data.launches[0].location.pads[0].latitude,
      data.launches[0].location.pads[0].longitude
    )
  );
...
function createLaunch(launch) {
  ...
  item.innerHTML =
    ...
    "</h1><a id='countdown_" +
    launch.id +
    "'></a><div class='description'><h1>Description</h1>" +
    ((launch.missions[0] && launch.missions[0].description) || "") +
    "</a>";
  if (
    ...
    item.style.backgroundImage =
      "linear-gradient(rgba(0,0,0,30), rgba(0,0,0,0)),url(" +
      launch.rocket.imageURL +
      ")";
  }

  launch.status == 1 || launch.status == 6
    ? Countdown(launch.net, document.querySelector("#countdown_" + launch.id))
    : $("#countdown_" + launch.id).html(statuses[launch.status].name);
  ...
  placemarkAttributes.labelAttributes.color =
    WorldWind.Color[statuses[launch.status].color];
